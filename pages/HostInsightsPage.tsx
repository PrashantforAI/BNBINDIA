...
const HostInsightsPage: React.FC<HostInsightsPageProps> = ({ navigate }) => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            data.conversations.forEach(c => c.participantIds.forEach(id => userIds.add(id)));
            data.reviews.forEach(r => userIds.add(r.guestId));
    ...
    if (loading) return <div>Loading AI Host Toolkit...</div>;
    if (!hostData) return <div>Could not load host data.</div>;

    return (
        <div>
            <h1 className="text-3xl font-bold mb-2 text-gray-50">AI Insights</h1>
